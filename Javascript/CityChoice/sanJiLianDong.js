function ThreeLevel(args){
	this.initProv=args.prov;
	this.initCity=args.city;
	this.initCounty=args.county;
	this.rootEle=document.getElementsByClassName("inner")[0];
	this.choProIndex=0;
	this.choCityIndex=0;
	this.type=args.type;
	this.init=function(){
		this.initSel(this.initProv);
		var sel=document.body.querySelector('div.inner>select:first-child');
		sel.addEventListener('change',this.chooseProv.bind(this));
		this.initSel(this.initCity);
		this.initSel(this.initCounty);
		var sel2=document.body.querySelector('div.inner>select:nth-child(2)');
		sel2.addEventListener('change',this.chooseCity.bind(this));
	};
	this.initSel=function(area){
		var sel=document.createElement("select");
		var opt=document.createElement("option");
		opt.innerHTML=area;
		opt.selected=true;
		sel.appendChild(opt);
		if(area==this.initProv){
			this.addRow(mapCityChoice,sel);
		}else if(this.type==2){
			sel.style.visibility='hidden';
		}
		this.rootEle.appendChild(sel);
		
	};
	this.chooseProv=function(){
		var opts=document.body.querySelector('div.inner>select:first-child');
		this.choProIndex=opts.selectedIndex-1;
		if(this.choProIndex>=0){
			var frag=document.createDocumentFragment();
			var arr=mapCityChoice[this.choProIndex].city;
			var sel=document.getElementsByTagName("select")[1];
			this.addRow(arr,frag);
			this.deleteRow(sel);
			sel.appendChild(frag);
		}
		if(this.type==2){
			var opts=document.body.querySelector('div.inner>select:nth-child(2)');
			opts.style.visibility='visible';
		}
	};
	this.chooseCity=function(){
		var opts=document.body.querySelector('div.inner>select:nth-child(2)');
		this.choCityIndex=opts.selectedIndex-1;
		if(this.choCityIndex>=0){
			var frag=document.createDocumentFragment();
			var arr=mapCityChoice[this.choProIndex].city[this.choCityIndex].area;
			var sel=document.getElementsByTagName("select")[2];
			this.addRow(arr,frag);//arr.name没有
			this.deleteRow(sel);
			sel.appendChild(frag);
		}
		if(this.type==2){
			var sel=document.body.querySelector('div.inner>select:nth-child(3)');
			sel.style.visibility='visible';
		}
	};
	this.addRow=function(arr,sel){
		for(var i=0;i<arr.length;i++){
			var opts=document.createElement("option");
			if(arr[i].name===undefined){
				opts.innerHTML=arr[i];
				opts.value=arr[i];
			}else{
				opts.innerHTML=arr[i].name;
				opts.value=arr[i].name;
			}
			sel.appendChild(opts);
		}
	};
	this.deleteRow=function(sel){
		if(sel.length>1){
...
window.onload=function(){
	//type=1时初始化三个选择框，type=2时只初始化一个选择框
	//new ThreeLevel({prov:'请选择省',city:'请选择市',county:'请选择县',type:'1'}).init();
	new ThreeLevel({prov:'请选择省',city:'请选择市',county:'请选择县',type:'2'}).init();
};
